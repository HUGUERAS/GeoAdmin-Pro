...
export default function AreaScreen() {
  ...
  const calcular = async () => {
    ...
    setResultado(null)
    try {
      const res = await fetch(`${API_URL}/geo/area`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ pontos: parsed }),
      })
  ...
  return (
    <ScrollView style={[s.container, { backgroundColor: C.background }]} keyboardShouldPersistTaps="handled">
      <View style={[s.header, { backgroundColor: C.card, borderBottomColor: C.cardBorder }]}>
        <Text style={[s.titulo, { color: C.text }]}>Área</Text>
        <Text style={[s.sub, { color: C.muted }]}>Área e perímetro de polígono UTM</Text>
      </View>
